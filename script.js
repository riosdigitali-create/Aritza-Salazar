...
<div class="diag-modal" id="diagModal" role="dialog" aria-modal="true" aria-labelledby="diagTitle">
    ...
    form.addEventListener('submit',e=>{
        ...
        const msg=`Hola Aritza, soy ${nombre}. Mi teléfono es ${telefono} y mi negocio es de ${giro}. Me gustaría agendar un diagnóstico estratégico.`;
...
/* ═══════════════════════════════════════════
   RESOURCE DOWNLOAD — captura email y envía a Aritza
═══════════════════════════════════════════ */
(function(){
    document.querySelectorAll('.recurso').forEach(card=>{
        ...
        form.addEventListener('submit',e=>{
            ...
            if(!email)return;
            // mailto fallback to personal inbox (Aritza puede reemplazar el destinatario)
            ...
            // Opens user's mail client to send to Aritza's inbox
            window.location.href=`mailto:aritza.anahisalazar@gmail.com?subject=${subject}&body=${body}`;
